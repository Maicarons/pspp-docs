...
When `N OF CASES` is specified after `TEMPORARY`, it affects only the next procedure (see [TEMPORARY](TEMPORARY.md)). Otherwise, cases beyond the limit specified are not processed by any later procedure. 
