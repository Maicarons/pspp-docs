...
`SHOW` can be used to display the current state of PSPP’s execution parameters. Parameters that can be changed using `SET` (see [SET](SET.md)), can be examined using `SHOW` using the subcommand with the same name. `SHOW` supports the following additional subcommands: 
...
Shows information about how PSPP was built. This information is useful in bug reports. See [Bugs](Bugs.md), for details. 
...
Display the terms of PSPP’s copyright licence (see [License](License.md)). 
