...
As you can see in Results 12.3 before the `FLIP` command has run there are seven variables (six containing data and one for the heading) and three cases. Afterwards there are four variables (one per case, plus the **CASE_LBL** variable) and six cases. You can delete the **CASE_LBL** variable (see [DELETE VARIABLES](DELETE-VARIABLES.md)) if you don’t need it. 
