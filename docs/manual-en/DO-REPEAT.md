...
New variable names used as replacements are not automatically created as variables, but only if used in the code block in a context that would create them, _e.g._ on a `NUMERIC` or `STRING` command or on the left side of a `COMPUTE` assignment. 
